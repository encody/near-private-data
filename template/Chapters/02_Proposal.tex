...
\begin{figure}[h!]
    ...
    % Add legend
    % \node[description] at (-4,6) {
    %     \begin{tabular}{l l}
    %         \raisebox{-1ex}{\tikz\node[private,minimum width=8mm] {};} & Private elements \\[0.5ex]
    %         \raisebox{-1ex}{\tikz\node[public,minimum width=8mm] {};} & Public elements
    %     \end{tabular}
    % };
    ...
    \node[user] (user) {
        \begin{tikzpicture}[node distance=1cm]
            ...
            \draw[->,solid] (key) .. controls +(3,-2) and +(0,1) .. (proof);
            \draw[->,solid] (hash) -- (proof) node[midway,left] {\textit{Proof generation}};
            \draw[->,solid] (encrypted1) -- (proof);

            \node[rectangle,draw=black,dotted,rounded corners=2em,minimum width=4cm,minimum height=2cm] (network) at ($(proof)+(+5,-1)$) {Proxy network};

            \node[enveloped] (onionpayload) at ($(proof)+(0,-2)$) {$(h^n, c^n, \pi^n)$};
            \draw[->,solid] (hash) -- (onionpayload);
            \draw[->,solid] (proof) -- (onionpayload) node[midway,left] {\textit{Layered routing encryption}};
            \draw[->,solid] (encrypted1) -- (onionpayload);
            \draw[->,solid] (network.west) .. controls +(-2,0) .. ($(onionpayload.north)+(.5,0)$);

            \draw[->,solid] (onionpayload) .. controls +(0,-2) and +(-1,-2) .. (network) node[midway,right] {\textit{Publish}};
    ...
    \end{tikzpicture}
    \caption{Message chunking, encryption, sequence hash generation, proving, and proxy routing.}
